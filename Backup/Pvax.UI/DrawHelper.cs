...
namespace Pvax.UI
{
	...
	public sealed class DrawHelper
	{
		...
		{
			if(null == brush)
				throw new ArgumentNullException("brush", RD.GetString("errNullParam"));
		...
		{
			if(null == image)
				throw new ArgumentNullException("image", RD.GetString("errNullParam"));
		...
		{
			if(null == path)
				throw new ArgumentNullException("path", RD.GetString("errNullParam"));
		...
		/// </summary>
		/// <returns>
		/// An exact copy of <see cref="StringFormat.GenericDefault"/> that can
